...
@interface STMutiTabView ()<UITableViewDelegate, UITableViewDataSource>{
    NSUInteger _selectedIndex;
    UIColor *_selectedColor;          //标题选定及指示器颜色
    UIColor *_titleColor;             //标题未选定颜色
    UIColor *_themeColor;             //页面背景色
    STMutiTabViewStyle _style;
}
...
-(instancetype)init {
    if (self = [super init]) {
        ...
        self.style = STMutiTabViewStyleTopTab;
    }
    return self;
}
...
-(void)setTitles:(NSArray<NSString *> *)titles {
    _titles = titles;
    [self.topTab removeAllSubviews];
    [self.bottomTab removeAllSubviews];
    ...
    for (NSUInteger i = 0; i<self.tabCount; ++i) {
        ...
        tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(_refresh:)];
        tableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(_loadMore:)];
...
- (void)setStyle:(STMutiTabViewStyle)style {
    _style = style;
    [self.topAssistView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.top.right.equalTo(self);
        make.height.mas_equalTo((style & STMutiTabViewStyleTopTab) ? @(45*kSCREEN_RATE) :@0);
    }];
    [self.bottomAssistView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.bottom.right.equalTo(self);
        make.height.mas_equalTo((style & STMutiTabViewStyleBottomButton) ? @(45*kSCREEN_RATE) : @0);
    }];
}
...
- (void)_refresh:(MJRefreshHeader *)sender {
    for (NSUInteger index = 0; index < self.tabCount; ++index) {
        if (sender == self.tableViews[index].mj_header) {
            self.tableViews[index].mj_footer.tag = 0;
            if ([self.delegate respondsToSelector:@selector(mutiTabView:beginToRefreshAtTabIndex:)]) {
                [self.delegate mutiTabView:self beginToRefreshAtTabIndex:index];
            } else {
                [self.tableViews[index] endRefreshing];
            }
        }
    }
}
- (void)_loadMore:(MJRefreshFooter *)sender {
    for (NSUInteger index = 0; index < self.tabCount; ++index) {
        if (sender == self.tableViews[index].mj_footer
            && [self.delegate respondsToSelector:@selector(mutiTabView:beginToLoadMoreInPage:atTabIndex:)]) {
            [self.delegate mutiTabView:self beginToLoadMoreInPage:++sender.tag atTabIndex:index];
        } else {
            [self.tableViews[index] endRefreshing];
        }
    }
...
@implementation UITableView(STMutiTabView)
